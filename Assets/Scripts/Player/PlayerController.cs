...
public class PlayerController : NetworkBehaviour, IPlayerActions {
	...
	public async Task SetPlayerName() {
		...
		GetComponent<PlayerData>().SetName(result);
	}
	...
	private void LateUpdate() {
		...
		Vector2 targetDirection = new Vector2(screenToWorldPosition.x - turretPivotTransform.position.x,
			screenToWorldPosition.y - turretPivotTransform.position.y).normalized;
